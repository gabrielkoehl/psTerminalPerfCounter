...
# ============================================================================
# Wir brauchen zwei externe Bibliotheken:
#   - NStack.dll:       Hilfs-Bibliothek fuer Terminal.Gui (String-Handling)
#   - Terminal.Gui.dll: Die eigentliche TUI-Engine (Fenster, Buttons, Labels, etc.)
#   - psTPCCLASSES.dll: Unsere eigene Klasse "CounterConfiguration" mit Datenmodell

# Pfad zum Lib-Ordner relativ zum Skript aufloesen
...
# Sicherheitscheck: Wenn die DLL fehlt, abbrechen
if (-not (Test-Path $termGuiDll)) {
    Write-Error "Terminal.Gui.dll nicht gefunden in: $tuiLibDir"
    return
}

# DLLs in den aktuellen Prozess laden (via Reflection, da sie keine PowerShell-Module sind)
...
# Unsere eigene C#-Klassen-DLL laden (enthaelt CounterConfiguration mit HistoricalData, Statistics, etc.)
...
if (Test-Path $pstpcDll) {
    # DLL existiert bereits vorkompiliert -> direkt laden
    Add-Type -Path (Resolve-Path $pstpcDll).Path
    Write-Host "psTPCCLASSES geladen."
} else {
    # DLL fehlt -> aus Quellcode bauen und dann laden
...
# ============================================================================
# SCHRITT 2: Fake-Counter erstellen (Simulierte Leistungsindikatoren)
# ============================================================================
# In der Produktion wuerden hier echte Windows Performance Counter abgefragt.
# Fuer die Demo erzeugen wir Fake-Objekte mit zufaelligen Werten.

function New-FakeCounter {
    param([string]$ComputerName, [string]$Title, [string]$Unit, [int]$DecimalPlaces = 1)

    # GetUninitializedObject erstellt ein Objekt OHNE den Konstruktor aufzurufen.
    # Das ist ein Trick, um die Klasse zu instanzieren, ohne alle Konstruktor-Parameter
    # liefern zu muessen - wir setzen die Eigenschaften manuell.
    $counter = [System.Runtime.Serialization.FormatterServices]::GetUninitializedObject([psTPCCLASSES.CounterConfiguration])
    $counter.ComputerName     = $ComputerName      # Servername, z.B. "SRV-DB01"
    $counter.Title            = $Title              # Counter-Name, z.B. "CPU"
    $counter.Unit             = $Unit               # Einheit, z.B. "%", "MB", "MB/s"
    $counter.DecimalPlaces    = $DecimalPlaces      # Nachkommastellen fuer Anzeige
    $counter.MaxHistoryPoints = 100                 # Maximal gespeicherte Datenpunkte
    $counter.HistoricalData   = [System.Collections.Generic.List[psTPCCLASSES.CounterConfiguration+DataPoint]]::new()  # Leere Liste fuer Zeitreihendaten
    $counter.Statistics       = [System.Collections.Generic.Dictionary[string, object]]::new()                         # Leeres Dictionary fuer Min/Max/Avg/etc.
    $counter.IsAvailable      = $true               # Counter ist verfuegbar
    $counter.LastError        = ''                   # Kein Fehler
    return $counter
}

# 4 simulierte Counter erstellen: CPU und RAM fuer DB-Server, CPU und Disk fuer Web-Server
...
# Jeden Counter mit 20 zufaelligen Start-Datenpunkten befuellen,
# damit die Sparklines sofort etwas anzeigen koennen
foreach ($c in $counters) {
    for ($i = 0; $i -lt 20; $i++) {
        # Realistische Wertebereiche je nach Counter-Typ
        switch ($c.Title) {
            "CPU"              { $val = Get-Random -Minimum 10.0 -Maximum 80.0 }       # CPU-Last: 10-80%
            "Available Memory" { $val = Get-Random -Minimum 1500.0 -Maximum 3500.0 }   # Freier RAM: 1.5-3.5 GB
            "Disk Read"        { $val = Get-Random -Minimum 0.5  -Maximum 50.0 }       # Disk I/O: 0.5-50 MB/s
            default            { $val = Get-Random -Minimum 1.0  -Maximum 100.0 }      # Fallback
        }
        # AddDataPoint() fuegt den Wert zur HistoricalData-Liste hinzu
        # und aktualisiert automatisch die Statistics (Min, Max, Avg, etc.)
...
# ============================================================================
# SCHRITT 3: Hilfsfunktionen fuer Textausrichtung in der Tabelle
# ============================================================================

function Format-Cell {
    # Formatiert einen Text auf eine feste Breite mit Polsterung (Padding).
    # Standard: rechtsbuendig (Zahlen). Mit -Left: linksbuendig (Text).
    # Fuegt links und rechts je ein Leerzeichen als Abstand hinzu.
...
function Format-Center {
    # Zentriert einen Text innerhalb einer festen Breite.
    # Wird fuer die Tabellen-Header benutzt.
    param([string]$Text, [int]$Width)
    if ($Text.Length -ge $Width) { return $Text.Substring(0, $Width) }   # Abschneiden falls zu lang
    $leftPad = [int][Math]::Floor(($Width - $Text.Length) / 2.0)        # Linke Polsterung berechnen
    $rightPad = $Width - $Text.Length - $leftPad                        # Rechte Polsterung = Rest
